    ...
    <Card className="p-4" hover>
      <div className="flex items-center justify-between mb-4">
        ...
        <button
          ...
          className="p-2 rounded-lg hover:bg-bg-secondary disabled:opacity-50"
        >
    ...
    <Card className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <div className="p-3 rounded-lg bg-bg-secondary text-center">
          <p className="text-xs text-text-muted">Avg Views/Video</p>
          <p className="text-lg font-bold">{stats.avgViews.toFixed(2)}</p>
        </div>
        <div className="p-3 rounded-lg bg-bg-secondary text-center">
          <p className="text-xs text-text-muted">Avg Likes/Video</p>
          <p className="text-lg font-bold">{stats.avgLikes.toFixed(2)}</p>
        </div>
        <div className="p-3 rounded-lg bg-bg-secondary text-center">
          <p className="text-xs text-text-muted">Avg Comments/Video</p>
      ...
      {filteredVideos.length === 0 ? (
        ...
        <div className="space-y-3 max-h-96 overflow-y-auto">
          {filteredVideos.slice(0, 20).map((video) => (
            <div 
              key={video.id} 
              className="flex gap-4 p-3 rounded-lg bg-bg-secondary hover:bg-bg-hover transition-colors cursor-pointer"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-text-muted">Total Engagement</p>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm text-text-muted">Avg. Engagement Rate</p>
            ...
            <Card className="p-6">
              ...
              <div className="flex items-center justify-between mt-4 text-sm text-text-muted">
                ...
                <span className={clsx(
                  ...
                  trend.isPositive ? 'text-green-600' : trend.value < 0 ? 'text-red-600' : 'text-text-muted'
                )}>
            ...
            {(youtubeAnalytics && (youtubeAnalytics.overview || youtubeAnalytics.trafficSources || youtubeAnalytics.topCountries)) && (
              <Card className="p-4">
                ...
                {youtubeAnalytics.trafficSources?.length > 0 && (
                  <div className="mb-2">
                    <p className="text-[10px] font-medium text-text-muted mb-1">TRAFFIC</p>
                    <div className="space-y-0.5">
                      {youtubeAnalytics.trafficSources.slice(0, 4).map((item: any, idx: number) => (
                        <div key={idx} className="flex items-center justify-between text-xs">
                          <span className="text-text-secondary capitalize truncate mr-1">
                ...
                {youtubeAnalytics.topCountries?.length > 0 && (
                  <div>
                    <p className="text-[10px] font-medium text-text-muted mb-1">COUNTRIES</p>
                    <div className="flex flex-wrap gap-1">
                      {youtubeAnalytics.topCountries.slice(0, 5).map((item: any, idx: number) => (
                        <span key={idx} className="px-1.5 py-0.5 text-[10px] rounded bg-bg-secondary">
                          {item.country}
                        </span>
                      ))}
                    </div>

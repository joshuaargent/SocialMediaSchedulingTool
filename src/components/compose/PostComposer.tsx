...
export function PostComposer({ onClose, onPublish, initialDate }: PostComposerProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-2xl bg-bg-card rounded-xl shadow-lg max-h-[90vh] overflow-hidden flex flex-col">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            {connectedPlatforms.length === 0 ? (
              <div className="p-6 rounded-lg border-2 border-dashed border-border text-center">
                ...
                <p className="text-text-secondary mb-3">No platforms connected</p>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2 rounded-lg border border-border bg-bg-card focus:ring-2 focus:ring-text-primary"
              >
                {postTypes.map((type) => (
          ...
          <div>
            ...
            <textarea
              ...
              rows={5}
              className="w-full px-4 py-3 rounded-lg border border-border bg-bg-card focus:ring-2 focus:ring-text-primary resize-none"
            />
          ...
          <div>
            ...
            <div 
              onClick={() => fileInputRef.current?.click()}
              className="border-2 border-dashed border-border rounded-lg p-8 text-center hover:border-text-primary transition-colors cursor-pointer"
            >
              <Image className="w-10 h-10 mx-auto mb-3 text-text-muted" />
            ...
            {mediaUrls.length > 0 && (
              <div className="grid grid-cols-4 gap-2 mt-3">
                {mediaUrls.map((url, idx) => (
                  <div key={idx} className="relative aspect-video rounded-lg overflow-hidden bg-bg-secondary">
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              ...
              {['Tomorrow 9 AM', 'Tomorrow 12 PM', 'Tomorrow 7 PM'].map((time) => (
                <button
                  ...
                  className="text-xs px-2 py-1 rounded bg-bg-secondary hover:bg-border-border transition-colors"
                >
                  {time}
                </button>
              ))}
          ...
          {isEvergreen && (
            <div className="flex items-center gap-3">
              ...
              <input
                ...
                className="w-20 px-3 py-2 rounded-lg border border-border"
              />
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border border-border bg-bg-card focus:ring-2 focus:ring-text-primary"
            />
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestedHashtags.map((tag) => (
                    <button
                      ...
                      className="px-3 py-1 rounded-full bg-bg-secondary text-sm hover:bg-text-primary hover:text-white transition-colors"
                    >
                      + {tag}
                    </button>
                  ))}
        ...
        {/* Footer */}
        <div className="flex items-center justify-between p-4 border-t border-border bg-bg-secondary/50">
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-bg-secondary transition-colors disabled:opacity-50"
            >
      ...
      {showPreview && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-3xl bg-bg-card rounded-xl shadow-lg max-h-[90vh] overflow-hidden">
            <div className="flex items-center justify-between p-4 border-b border-border">
              <h2 className="text-lg font-semibold">Post Preview</h2>
              <button onClick={() => setShowPreview(false)} className="p-1.5 rounded-lg hover:bg-bg-secondary">
                <X className="w-5 h-5" />
              </button>
            </div>
                      ...
                      {preview.hasMedia && (
                        <div className="grid grid-cols-2 gap-1 p-3 bg-black/5">
                          {mediaUrls.slice(0, 4).map((url, idx) => (
                            <div key={idx} className="aspect-video rounded bg-bg-secondary overflow-hidden">
                              <img src={url} alt="" className="w-full h-full object-cover" />
                            </div>
                          ))}
                          {mediaUrls.length > 4 && (
                            <div className="aspect-video rounded bg-bg-secondary flex items-center justify-center">
                              <span className="text-text-muted">+{mediaUrls.length - 4}</span>
                            </div>
                          )}
                        </div>
                      )}
